...
interpret graph les deux RF
key takeaways
future R
recommandations cours datacamp tree based models; workspace pipes


ADD TABLE OF CONTENT IN FIRST SECTION WHEN IM DONE

ajouter github a la fin
...
In this particular case, with no hyperparameter tuning in the random forests, the addition of a Gower neighbor with optimized weights yielded similar results to the random forest witout feature engineering. Looking at variable importances, it looks like `nn_p`, namely the Gower distance that we transformed into an approximate probability earlier, became one of the strongest predictors of the target in this dataset:
...
* several nearest neighbors and take modal target value (if numeric target: average or median)
* optim() to find true optimized combination of weights, with continuous weights instead of dichotomous 0 or 1
* with missing data and lots of factor levels where gower shines
* tuning model hyperparameters (like mtry in RF) separately for RF with and without neighbor feature, so that they both are optimized for the features they do have



## Code Repository

The entire code repository for this project is available on GitHub.

[![Code Repository on GitHub](https://i.imgur.com/kDrGrBN.png)](https://github.com/patc3-phd/datacamp-loans-competition)
